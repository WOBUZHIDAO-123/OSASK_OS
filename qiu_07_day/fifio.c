...
// ============================================================
// fifo8_get — 从 FIFO8 环形缓冲区读取一个字节
//
// 功能：从缓冲区头部（q 指向的位置）读取一个字节并移除。
//       若缓冲区为空，返回 -1 表示无数据可读。
//
// 环形机制：
//   q 在读取后自增，到达 size 边界时回绕到 0。
//   free 递增反映空间释放（free == size 表示空）。
//
// 参数：
//   fifo — 指向 FIFO8 结构体
//
...
// ============================================================
// fifo8_status — 查询 FIFO8 环形缓冲区当前数据量
//
// 功能：返回缓冲区中当前已写入但尚未读取的字节数。
//       相当于「队列长度」，可用于判断是否需要处理数据。
//
// 参数：
//   fifo — 指向 FIFO8 结构体
//
