 ...
 * bootpack.c — 内核主函数
 *
 * 第7天新增功能：键盘按键显示 + 鼠标数据读取
 *   在屏幕左上角两行分别显示键盘扫描码和鼠标数据字节。
 *   键盘中断 → keyfifo（IRQ1）
...
void HariMain(void)
{
	...
	//   ④ 鼠标有数据 → fifo8_get 读取 → STI → 在右侧显示数据字节
	//
	// 为什么要先屏蔽 PIC 中断信号再检查？
	...
	for (;;)
	{
		...
		else
		{
			...
			{
				int i = fifo8_get(&mousefifo); // 从鼠标 FIFO 取出一个字节（鼠标原始数据）
				io_sti();					   // 恢复 PIC 信号响应（显示操作允许被打断）
				// 在屏幕左上角第二行右侧（x=32, y=16）显示鼠标数据
				sprintf(s, "%02X", i);											   // 格式化为 2 位 16 进制
				boxfill8(binfo->vram, binfo->scrnx, COL8_008484, 32, 16, 47, 31); // 清空鼠标显示区域
				putfonts8_asc(binfo->vram, binfo->scrnx, 32, 16, COL8_FFFFFF, s); // 显示鼠标数据
			}
		}
	}
}
